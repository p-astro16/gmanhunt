...
function Manhunt.DrawVictoryScreen(scrW, scrH)
    ...
    draw.SimpleText(winnerText, "Manhunt_Countdown", centerX, centerY - 120, winnerColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    
    -- Reason
    draw.SimpleText(Manhunt.VictoryReason or "", "Manhunt_Alert", centerX, centerY - 40, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    
    -- Match Statistics
    if Manhunt.MatchStats and table.Count(Manhunt.MatchStats) > 0 then
        local statsY = centerY + 20
        local lineHeight = 30
        
        draw.SimpleText("MATCH STATISTICS", "Manhunt_Label", centerX, statsY, COLOR_YELLOW, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
        statsY = statsY + lineHeight + 10
        
        -- Bullets fired
        draw.SimpleText("Bullets Fired (Hunter): " .. (Manhunt.MatchStats.BulletsFired or 0), "Manhunt_Label", centerX, statsY, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
        statsY = statsY + lineHeight
        
        -- Damage dealt
        draw.SimpleText("Damage Dealt (Hunter): " .. string.format("%.1f", Manhunt.MatchStats.DamageDealt or 0), "Manhunt_Label", centerX, statsY, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
        statsY = statsY + lineHeight
        
        -- Distance traveled
        local hunterDist = Manhunt.MatchStats.HunterDistanceTraveled or 0
        local fugitiveDist = Manhunt.MatchStats.FugitiveDistanceTraveled or 0
        draw.SimpleText("Distance Traveled (Hunter): " .. string.format("%.1f", hunterDist / 39.37) .. "m", "Manhunt_Label", centerX, statsY, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
        statsY = statsY + lineHeight
        
        draw.SimpleText("Distance Traveled (Fugitive): " .. string.format("%.1f", fugitiveDist / 39.37) .. "m", "Manhunt_Label", centerX, statsY, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
        statsY = statsY + lineHeight
        
        -- Closest distance
        local closestDist = Manhunt.MatchStats.ClosestDistance or 999999
        if closestDist < 999999 then
            draw.SimpleText("Closest Distance: " .. string.format("%.1f", closestDist / 39.37) .. "m", "Manhunt_Label", centerX, statsY, COLOR_GREEN, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
        end
    end
    ...
    if resetTime > 0 then
        draw.SimpleText("Resetting in " .. resetTime .. "...", "Manhunt_Label", centerX, scrH - 100, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
end
