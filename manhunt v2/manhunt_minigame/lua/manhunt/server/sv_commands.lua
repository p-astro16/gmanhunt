...
concommand.Add("manhunt_start", function(ply, cmd, args)
    ...
    if #args < 3 then
        ...
        usage = usage .. "  time: Game duration in seconds (10-3600)\n"
        usage = usage .. "  interval: Ping interval in seconds (5-60)\n"
    ...
    -- Validate duration (minimum 10 seconds for testing, max 1 hour)
    if not duration or duration < 10 or duration > 3600 then
        local msg = "[Manhunt] Invalid duration. Must be between 10 and 3600 seconds (1 hour)"
        if IsValid(ply) then
    ...
    -- Validate interval (minimum 5 seconds for testing)
    if not interval or interval < 5 or interval > 60 then
        local msg = "[Manhunt] Invalid interval. Must be between 5 and 60 seconds"
        if IsValid(ply) then
            ply:PrintMessage(HUD_PRINTCONSOLE, msg)
        else
            print(msg)
