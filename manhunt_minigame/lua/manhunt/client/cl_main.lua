...
Manhunt.SpeedBoostActive = false
...
net.Receive("manhunt_speed_boost", function()
    local active = net.ReadBool()
    Manhunt.SpeedBoostActive = active
    
    if active then
        surface.PlaySound("items/battery_pickup.wav")
        chat.AddText(Color(100, 255, 100), "[Manhunt] ", Color(255, 255, 255), "Speed boost activated!")
    else
        chat.AddText(Color(150, 150, 150), "[Manhunt] ", Color(255, 255, 255), "Speed boost expired")
    end
    
    print("[Manhunt] Speed boost: " .. (active and "Active" or "Expired"))
end)
